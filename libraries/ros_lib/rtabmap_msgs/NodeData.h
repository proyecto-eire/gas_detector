#ifndef _ROS_rtabmap_msgs_NodeData_h
#define _ROS_rtabmap_msgs_NodeData_h
...
namespace rtabmap_msgs
{
  ...
  class NodeData : public ros::Msg
  {
    public:
      ...
      typedef rtabmap_msgs::KeyPoint _wordKpts_type;
      _wordKpts_type st_wordKpts;
      _wordKpts_type * wordKpts;
      uint32_t wordPts_length;
      typedef rtabmap_msgs::Point3f _wordPts_type;
      ...
      typedef rtabmap_msgs::GlobalDescriptor _globalDescriptors_type;
    ...
    NodeData():
      ...
      baseline_length(0), st_baseline(), baseline(nullptr),
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(wordKpts_lengthT > wordKpts_length)
        this->wordKpts = (rtabmap_msgs::KeyPoint*)realloc(this->wordKpts, wordKpts_lengthT * sizeof(rtabmap_msgs::KeyPoint));
      wordKpts_length = wordKpts_lengthT;
      for( uint32_t i = 0; i < wordKpts_length; i++){
      offset += this->st_wordKpts.deserialize(inbuffer + offset);
        memcpy( &(this->wordKpts[i]), &(this->st_wordKpts), sizeof(rtabmap_msgs::KeyPoint));
      }
      ...
      if(wordPts_lengthT > wordPts_length)
        this->wordPts = (rtabmap_msgs::Point3f*)realloc(this->wordPts, wordPts_lengthT * sizeof(rtabmap_msgs::Point3f));
      wordPts_length = wordPts_lengthT;
      for( uint32_t i = 0; i < wordPts_length; i++){
      offset += this->st_wordPts.deserialize(inbuffer + offset);
        memcpy( &(this->wordPts[i]), &(this->st_wordPts), sizeof(rtabmap_msgs::Point3f));
      }
      ...
      if(globalDescriptors_lengthT > globalDescriptors_length)
        this->globalDescriptors = (rtabmap_msgs::GlobalDescriptor*)realloc(this->globalDescriptors, globalDescriptors_lengthT * sizeof(rtabmap_msgs::GlobalDescriptor));
      globalDescriptors_length = globalDescriptors_lengthT;
      for( uint32_t i = 0; i < globalDescriptors_length; i++){
      offset += this->st_globalDescriptors.deserialize(inbuffer + offset);
        memcpy( &(this->globalDescriptors[i]), &(this->st_globalDescriptors), sizeof(rtabmap_msgs::GlobalDescriptor));
      }
    ...
    virtual const char * getType() override { return "rtabmap_msgs/NodeData"; };
    virtual const char * getMD5() override { return "bbf7b81adbf2e3b56c6bae97e128cd38"; };
