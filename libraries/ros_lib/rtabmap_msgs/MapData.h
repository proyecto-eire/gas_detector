...
namespace rtabmap_msgs
{
  ...
  class MapData : public ros::Msg
  {
    public:
      ...
      typedef rtabmap_msgs::NodeData _nodes_type;
    ...
    virtual int deserialize(unsigned char *inbuffer) override
    {
      ...
      if(nodes_lengthT > nodes_length)
        this->nodes = (rtabmap_msgs::NodeData*)realloc(this->nodes, nodes_lengthT * sizeof(rtabmap_msgs::NodeData));
      nodes_length = nodes_lengthT;
      for( uint32_t i = 0; i < nodes_length; i++){
      offset += this->st_nodes.deserialize(inbuffer + offset);
        memcpy( &(this->nodes[i]), &(this->st_nodes), sizeof(rtabmap_msgs::NodeData));
      }
     return offset;
    }

    virtual const char * getType() override { return "rtabmap_msgs/MapData"; };
    virtual const char * getMD5() override { return "e20572a812f1f74060a7f5d1a39f9106"; };
